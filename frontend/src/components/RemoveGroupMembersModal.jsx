...
const RemoveGroupMembersModal = () => {

    const {setModalType, setIsModalOpen, setSelectedMembers, selectedMembers, removeMembersFromGroup, selectedGroup} = useContext(GroupChatContext);
    ...
    const handleRemoveMembers = async (e) => {
        ...
        try {
            if(selectedMembers.length > 0) {
                await removeMembersFromGroup(selectedGroup._id, {members:selectedMembers});
                setModalType(null);
                setIsModalOpen(false);
                setSearchText("");
                setSelectedMembers([]);
    ...
    const availableMembers = selectedGroup.groupMembers.filter((member) => member._id.toString() !== selectedGroup.groupAdmin._id.toString());
        ...
        <div className="fixed inset-0 bg-black/60 w-full h-screen flex items-center justify-center z-50 backdrop-blur-md p-4">
            <div className="bg-[#f9fafc] rounded-2xl shadow-2xl h-[75dvh] md:h-[85vh] w-full max-w-lg overflow-hidden p-4 md:p-6 flex flex-col">
                <div className="flex justify-between items-start mb-4 shrink-0">
                    <div>
                        <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 tracking-tight">Remove Members</h2>
                        <p className="text-gray-500 mt-0.5 text-xs md:text-sm">Select friends to remove from the group.</p>
                    </div>
                    <button onClick={handleCloseButton} className="text-gray-400 hover:text-gray-900 hover:bg-[#DCDCFE] p-2 rounded-full transition-all cursor-pointer">
                        <X size={20} />
                    </button>
                </div>
                <div className="flex-1 min-h-0 flex flex-col gap-3">
                    ...
                    <form onSubmit={handleRemoveMembers} className="flex-1 min-h-0 flex flex-col justify-between gap-3">
                        <div className="add-member-container flex-1 min-h-0 flex flex-col gap-2 overflow-y-auto pr-1">
                        ...
                        <div className="shrink-0 pt-4 border-t border-gray-100 flex justify-between items-center mt-3">
                            ...
                            <div className="flex gap-3">
                                <button onClick={handleCloseButton} type="button" className="px-4 py-2 rounded-md font-semibold text-gray-600 hover:bg-[#DCDCFE] hover:text-gray-900 transition-all cursor-pointer text-sm md:text-base">Cancel</button>
                                <button disabled={selectedMembers.length === 0} type="submit" className="px-5 py-2 bg-[#7678ed] hover:bg-[#5e5ebf] disabled:bg-[#7678ed]/50 disabled:cursor-not-allowed text-white font-medium rounded-md transition-all shadow-md cursor-pointer text-sm md:text-base">Remove Members</button>
                            </div>
                        </div>
                    </form>
                </div>
